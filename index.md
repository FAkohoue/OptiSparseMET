...
![OptiSparseMET logo](reference/figures/logo.svg)
...
These two levels are statistically linked. Allocation decisions
constrain which field layouts are feasible within environments; field
layout choices affect the precision with which allocation-level effects
can be estimated. Optimizing each level in isolation is not equivalent
to joint optimization. `OptiSparseMET` formalizes the link between them.
...
The theoretical basis follows Montesinos-López et al. (2023), who
...
**family-based allocation** that distributes each family group across
environments; and **GRM/A-based allocation** that uses genomic or
pedigree relationships to prevent genetic clustering across
environments.
...
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|
| [`allocate_sparse_met()`](https://FAkohoue.github.io/OptiSparseMET/reference/allocate_sparse_met.md)                                     | Distribute treatments across environments (M3 or M4)    |
| [`check_balanced_incomplete_feasibility()`](https://FAkohoue.github.io/OptiSparseMET/reference/check_balanced_incomplete_feasibility.md) | Verify BIBD feasibility given trial dimensions          |
| [`derive_allocation_groups()`](https://FAkohoue.github.io/OptiSparseMET/reference/derive_allocation_groups.md)                           | Derive grouping structure from family, GRM, or A matrix |
...
| [`min_k_for_full_coverage()`](https://FAkohoue.github.io/OptiSparseMET/reference/min_k_for_full_coverage.md) | Compute minimum capacity for full treatment coverage  |
...
| Function                                                                                     | Description                                              |
|----------------------------------------------------------------------------------------------|----------------------------------------------------------|
| [`prep_famoptg()`](https://FAkohoue.github.io/OptiSparseMET/reference/prep_famoptg.md)       | Augmented, p-rep, or repeated-check block designs        |
| [`alpha_rc_stream()`](https://FAkohoue.github.io/OptiSparseMET/reference/alpha_rc_stream.md) | Alpha row-column designs for fixed-grid field deployment |

### Pipeline and assembly

| Function                                                                                                   | Description                                                 |
|------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|
| [`plan_sparse_met_design()`](https://FAkohoue.github.io/OptiSparseMET/reference/plan_sparse_met_design.md) | End-to-end pipeline in a single call                        |
...
some stochasticity in allocation is acceptable.

**Use `balanced_incomplete`** when environments are comparable in size,
when equal replication is a hard requirement, and when the downstream
analysis relies on the precision guarantees that uniform co-occurrence
provides.
