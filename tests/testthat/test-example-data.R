...
test_that("OptiSparseMET_example_data treatments and environments are valid character vectors", {
  ...
  # Treatment IDs must be unique — duplicates would silently corrupt allocation
...
test_that("OptiSparseMET_example_data treatment_info has required columns and covers all treatments", {
  ...
  # Every treatment must have a row in treatment_info
  ...
  # Family column must be non-empty strings
...
test_that("OptiSparseMET_example_data seed_info covers all treatments with positive values", {
  ...
  # Every treatment must have a seed entry
  ...
  # Seed quantities must be strictly positive
  expect_true(
...
test_that("OptiSparseMET_example_data seed_required_per_plot covers all environments with positive values", {
  ...
  # Every environment must have a per-plot requirement
...
test_that("OptiSparseMET_example_data relationship matrices are square with correct IDs", {
  ...
  for (mat_name in c("OptiSparseMET_GRM", "OptiSparseMET_A", "OptiSparseMET_K")) {
    ...
    # Row and column names must match the treatment IDs exactly
    ...
    # Diagonal values must be positive (required for Cholesky factorization)
    expect_true(
      all(diag(mat) > 0),
...
test_that("OptiSparseMET_example_data sparse argument lists have required fields and consistent IDs", {
  ...
  for (args_name in c("sparse_example_args_random_balanced",
    ...
    # Treatment and environment IDs inside the arg list must match
    # the top-level components so the args can be used interchangeably
    expect_equal(args$treatments,   x$treatments,   info = paste(args_name, "treatments must match x$treatments"))
    expect_equal(args$environments, x$environments, info = paste(args_name, "environments must match x$environments"))
  }
  
  # allocation_method values must match their expected canonical names
  expect_equal(
...
test_that("OptiSparseMET_example_data env_design_specs covers all environments and has required fields", {
  ...
  # Every environment must have a spec
  ...
  for (env in x$environments) {
    ...
    # Every spec must have a design field with a supported value
    expect_true("design" %in% names(spec),
                info = paste("env_design_specs[[", env, "]] must have a 'design' field"))
    expect_true(
      spec$design %in% c("prep_famoptg", "alpha_rc_stream"),
      info = paste("env_design_specs[[", env, "]]$design must be 'prep_famoptg' or 'alpha_rc_stream'")
    )
    
    # Every spec must have check_treatments and check_families
