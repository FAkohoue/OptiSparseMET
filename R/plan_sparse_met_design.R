...
#' - `prep_famoptg()` replication mode can be inferred automatically when not
#'   supplied explicitly
...
#'   engine (`"prep_famoptg"` or `"alpha_rc_stream"`). All other fields are
#'   passed through to the corresponding local design function, except a small
#'   set of pipeline-consumed fields used internally for seed-aware replication.
...
plan_sparse_met_design <- function(
  ...
  resolve_seed_required <- function(seed_required_per_plot, env_name) {
    ...
    if (is.numeric(seed_required_per_plot) && length(seed_required_per_plot) == 1L) {
      if (is.na(seed_required_per_plot) || seed_required_per_plot <= 0) {
        return(default_val)
      }
    ...
    if (is.numeric(seed_required_per_plot) &&
      ...
      if (is.null(val) || is.na(val) || length(val) != 1L || val <= 0) {
        return(default_val)
      }
      return(val)
    }
  ...
  validate_env_spec <- function(spec, env_name) {
    ...
    if (is.character(design_name)) {
      design_name <- as.character(design_name)[1L]
    }
    
    if (identical(design_name, "prep_famoptg")) {
      req <- c("check_treatments", "check_families", "n_blocks", "n_rows", "n_cols")
      miss <- req[!req %in% names(spec)]
      if (length(miss) > 0L) {
        stop(
          "Environment `", env_name, "` using `prep_famoptg` is missing required fields: ",
          paste(miss, collapse = ", ")
        )
      }
    ...
    } else if (identical(design_name, "alpha_rc_stream")) {
      req <- c("check_treatments", "check_families", "n_reps", "n_rows", "n_cols")
      miss <- req[!req %in% names(spec)]
      if (length(miss) > 0L) {
        stop(
          "Environment `", env_name, "` using `alpha_rc_stream` is missing required fields: ",
          paste(miss, collapse = ", ")
        )
      }
      if (length(spec$check_treatments) != length(spec$check_families)) {
  ...
  infer_replication_mode <- function(spec, env_name, env_treatments) {
    if (!is.null(spec$replication_mode)) {
      return(as.character(spec$replication_mode)[1L])
    }
  ...
  build_prep_famoptg_args <- function(spec, env_name, env_treatments) {
    ...
    if (!is.numeric(seed_req) || length(seed_req) != 1L || is.na(seed_req) || seed_req <= 0) {
      stop(
        "Invalid seed_required_per_plot for environment ", env_name,
  ...
  build_alpha_rc_stream_args <- function(spec, env_treatments) {
    local_args <- spec
    local_args$design <- NULL
  ...
  build_generic_engine_args <- function(fun, spec, env_treatments) {
    ...
    if (length(trt_name) > 0L) {
      local_args[[trt_name[1L]]] <- env_treatments
    }
    if (length(fam_name) > 0L) {
      local_args[[fam_name[1L]]] <- get_family_vec(env_treatments)
    }
  ...
  resolve_design_engine <- function(design_spec) {
    ...
    builtins <- list(
      prep_famoptg = prep_famoptg,
      alpha_rc_stream = alpha_rc_stream
    )
  ...
  for (env_name in environments) {
    ...
    if (identical(design_name, "prep_famoptg")) {
      ...
      local_args <- build_alpha_rc_stream_args(spec, env_treatments)
    ...
    if (!is.list(local_out) || !"field_book" %in% names(local_out)) {
      stop(
        "Local design function for environment ", env_name,
  ...
  environment_summary <- environment_summary[
    match(environments, environment_summary$Environment),
    ,
    drop = FALSE
  ]
  rownames(environment_summary) <- NULL
