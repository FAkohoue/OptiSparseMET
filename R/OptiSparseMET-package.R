...
#' `prep_famoptg()` constructs augmented, partially replicated (p-rep), or
#' RCBD-type repeated-check designs for environments where block-based local
#' control is appropriate. `alpha_rc_stream()` constructs stream-based alpha
#' row-column designs for environments with fixed grid geometry and continuous
#' field-book order, accommodating AR1 and AR1×AR1 spatial models at the
#' analysis stage.
#'
#' When replication within an environment is flexible, `assign_replication_by_seed()`
#' evaluates seed availability against the per-plot seed requirement for each
#' treatment and assigns replication roles — replicated, unreplicated, or
#' excluded — before the field layout is constructed. Both within-environment
#' design functions accept optional relationship matrices for structure-aware
#' entry arrangement and dispersion optimization, and can evaluate the resulting
#' design under a mixed-model framework prior to field deployment.
#'
...
#' | `prep_famoptg()` | Build block-based within-environment layouts |
#' | `alpha_rc_stream()` | Build stream-based row-column layouts |
