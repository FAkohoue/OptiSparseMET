...
#' `n_test_entries_per_environment` that is too small to cover all treatments,
#' which causes [allocate_sparse_met()] to either error, produce incomplete
#' allocation, or require `allow_approximate = TRUE` in ways that leave some
#' treatments unassigned. Calling these helpers before allocation lets the user
#' verify feasibility and choose a safe capacity without trial and error.
#'
#' The three exported helpers serve distinct purposes.
#' [min_k_for_full_coverage()] computes the arithmetic minimum.
#' [suggest_safe_k()] adds a user-defined buffer on top of that minimum and
#' returns a single ready-to-use integer. [warn_if_k_too_small()] checks an
#' existing configuration and emits a warning with a corrective suggestion if
#' the capacity is insufficient. The internal function
#' `.check_full_coverage_feasibility()` is called by [allocate_sparse_met()]
...
#'     sparse-allocatable slots required per environment:
#'     \eqn{\lceil J^* / I \rceil}.}
...
#' }
#'
#' @seealso [suggest_safe_k()] which adds a buffer on top of this minimum.
#'   [warn_if_k_too_small()] for a non-fatal feasibility check.
#'   [allocate_sparse_met()] for the allocation function this feeds into.
#'
#' @examples
...
#'
#' @seealso [min_k_for_full_coverage()] for the strict minimum without a buffer.
#'   [warn_if_k_too_small()] to check an existing capacity non-fatally.
#'   [allocate_sparse_met()] for the allocation function this feeds into.
#'
#' @examples
...
suggest_safe_k <- function(
  ...
  if (length(environments) < 1L) stop("`environments` must contain at least one environment.")
...
#'
#' @seealso [suggest_safe_k()] and [min_k_for_full_coverage()] for functions
#'   that compute a feasible capacity. [allocate_sparse_met()] which enforces
#'   this condition with a hard stop via `.check_full_coverage_feasibility()`.
#'
#' @examples
#' trt <- paste0("L", sprintf("%03d", 1:80))
#' env <- c("Env1", "Env2", "Env3")
...
#' @param n_test_entries_per_environment Integer scalar or integer vector.
#' @param common_treatments Character vector of common treatment IDs.
#'
#' @return Invisibly returns `TRUE` when the condition is satisfied.
#'
#' @keywords internal
