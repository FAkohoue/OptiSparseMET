...
#'
#' @description
#' Environment-level field books produced by [met_prep_famoptg()] or
#' [met_alpha_rc_stream()] may differ in their column sets depending on the
#' design options used in each environment. A direct `rbind()` over such a list
#' fails unless all data frames share identical columns.
#' `combine_met_fieldbooks()` resolves this by computing the union of all
#' column names across environments, padding missing columns with `NA` in each
#' data frame before stacking, and placing a standard set of columns at the
#' front of the result regardless of the order in which they appear in the
#' individual field books.
#'
...
#'   the local design type used in each environment, typically
#'   `"met_prep_famoptg"` or `"met_alpha_rc_stream"` for built-in engines.
#'   Names must match the names of `field_books`. Environments not present in
#'   `local_designs` receive `NA` in the `LocalDesign` column. If `NULL`, all
#'   environments receive `NA`.
#'
...
#'   that environment.
#'
#' @seealso [met_prep_famoptg()] and [met_alpha_rc_stream()] which produce the
#'   environment-level field books that serve as input to this function.
#'   [plan_sparse_met_design()] which calls this function internally as the
#'   final step of the two-stage MET pipeline.
#'
#' @examples
...
combine_met_fieldbooks <- function(
  ...
  for (env in envs) {
    ...
    fb$IsCommonTreatment <- if (is.null(common_treatments)) FALSE else
      fb$Treatment %in% common_treatments
